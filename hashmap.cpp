...
/* Copy Construct Version 2 */
template <typename K, typename M, typename H>
HashMap<K, M, H>::HashMap(const HashMap& rhs): 
    _size(rhs._size),
    _hash_function(rhs._hash_function),
    _buckets_array(rhs.bucket_count(),nullptr) {
    auto this_bucket_itr = this->_buckets_array.begin();
    auto rhs_bucket_itr = rhs._buckets_array.begin();
    while(rhs_bucket_itr < rhs._buckets_array.end()){
        *this_bucket_itr = bucket_copy(*rhs_bucket_itr);
        ++this_bucket_itr;
        ++rhs_bucket_itr;
    }
}
...
template <typename K, typename M, typename H>
typename HashMap<K, M, H>::node* HashMap<K, M, H>::bucket_copy(
    HashMap<K, M, H>::node* head){
    node* new_head = nullptr;
    std::stack<value_type> tmp_stk;
    while(head != nullptr){
        tmp_stk.push(head->value);
        head = head->next;
    }
    while(!tmp_stk.empty()){
        auto curr = new node(tmp_stk.top(), new_head);
        new_head = curr;
        tmp_stk.pop();
    }
    
    /* check if list copy succeed 
    std::cout << "---------------checking for list copy---------------------"
              << std::endl;
    auto curr = head;
    while(curr != nullptr){
        const auto& [key, val] = curr->value;
        std::cout << " -> " << key << ":" << val;
        curr = curr->next;
    }
    std::cout << std::endl;
